...
template<class T> class Matrix_base {
    // matrixs store their memory (elements) in Matrix_base and have copy
    // semantics
...
public:
    ...
    void swap_rows(Index i, Index j)
        // swap_rows() uses a row's worth of memory for better run-time
        // performance if you want pairwise swap, just write it yourself
    {
        if (i == j) return;
    /*
...
A apply(F f, A x)        { A res(x,f);   return xfer(res); }
template<class F, class Arg, class A>
A apply(F f, A x, Arg a) { A res(x,f,a); return xfer(res); }
